...
public class GUI extends JFrame{
    ...
    /**
     * Setup GUI window & keep updating/display changes
     *
    ...
    public int[][] restartCell = new int[64][36];
    public boolean restartOrClear = false;
    ...
    public boolean randomTFBool = false;
    public boolean randomRun = false;

    public TextField randomCellTF = new TextField();
    public int textFLoop = 0;
    public String userInputRandomTF = "";
    public int userInputRandomTFInt = 0;
    ...
    public GUI(){
        ...
        randomCellTF.setBounds(980, 850, 150, 20);
        ...
        //add "board" into GUI window
        Board board = new Board();
        this.setContentPane(board);

        //add mouse listener that gets user input
        Move move = new Move();
    ...
    public class Board extends JPanel{
        ...
        public void paintComponent(Graphics g) {
            //check number of live cells beside
            for (int i = 0; i < 64; i++) {
                for (int j = 0; j < 36; j++) {
            ...
            if(numberOfGeneration == 0){
                restartOrClear = false;
            }else{
                restartOrClear = true;
            }

            //allows user to clear(all cells dead) the board
            if(clear){
                for(int i = 0;i < 64;i++){
                    for(int j = 0;j < 36;j++){
                        aliveInt[i][j] = 0;
            ...
            //allow user to reset from the 0th generation
            if(restart){
                for (int i = 0; i < 64; i++) {
                    for (int j = 0; j < 36; j++) {
                        aliveInt[i][j] = restartCell[i][j];
                    }
                }
                numberOfGeneration = 0;
                restart = false;
            ...
            //set alive cells color yellow and dead cells colour gray
            for (int i = 0; i < 64; i++) {
                for (int j = 0; j < 36; j++) {
                    if (aliveInt[i][j] == 1) {
            ...
            //change the colour of cells where users mouse on to remind them
            for(int i = 0; i < 64;i++){
                for (int j = 0; j < 36; j++) {
            ...
            //display/output different types of cell in game of life
            //lifes type
            if(lifes){
                for(int i = 0;i < 64;i++){
                    for(int j = 0;j < 36;j++){
            ...
            g.drawString("* From Wikipedia, Conway's Game of Life", 1292, 870);
            ...
            g.setFont(new Font("Tahoma", Font.PLAIN, 60));
            g.setColor(Color.white);
            if(restartOrClear == false) {
                g.drawString("CLEAR", 20, 60);
            }else if(restartOrClear){
                g.drawString("Restart", 15, 60);
            }
            ...
            g.setColor(Color.darkGray);
            if(mouseX > 987 && mouseX < 1187 && mouseY > 840 && mouseY < 900){
                g.setColor(Color.gray);
            }
            g.fillRect(980, 810, 200, 60);
            g.setColor(Color.white);
            ...
            g.drawString("Random", 997, 855);

            //draw random input button/text field
            if(randomTFBool){
                g.setColor(Color.gray);
                g.fillRect(980, 810, 200, 60);
                g.setColor(Color.darkGray);
                g.fillRect(1130, 850, 50, 20);
                g.setColor(Color.white);
                g.setFont(new Font("Tahoma", Font.PLAIN, 30));
                g.drawString("Live Cells:", 1000, 840);
                g.setFont(new Font("Tahoma", Font.PLAIN, 20));
                g.drawString("run", 1140, 865);
                if(textFLoop == 0) {
                    this.add(randomCellTF);
                    randomCellTF.setText("");
                    textFLoop++;
                }
            }
            if(!randomTFBool){
                textFLoop = 0;
                this.remove(randomCellTF);
                randomRun = false;
            }

            //validation
            //check user input if it's not null and it's an integer between 0~2304(64x36)
            if(randomRun){
                if(userInputRandomTF.isEmpty()){
                    if(randomTFBool) {
                        g.setColor(Color.gray);
                        g.fillRect(980, 810, 200, 40);
                        g.setColor(Color.white);
                        g.setFont(new Font("Tahoma", Font.PLAIN, 17));
                        g.drawString("Please Enter A Number:", 983, 837);
                    }
                }else {
                    try {
                        if (Integer.parseInt(userInputRandomTF) < 0 || Integer.parseInt(userInputRandomTF) > 2304) {
                            g.setColor(Color.gray);
                            g.fillRect(980, 810, 200, 40);
                            g.setColor(Color.white);
                            g.setFont(new Font("Tahoma", Font.PLAIN, 17));
                            g.drawString("Please Enter A Number", 983, 827);
                            g.drawString("Between 0~2304:", 983, 845);
                        } else {
                            userInputRandomTFInt = Integer.parseInt(userInputRandomTF);
                            //clear the board
                            for(int i = 0;i < 64;i++){
                                for (int j = 0; j < 36; j++) {
                                    aliveInt[i][j] = 0;
                        ...
                        if (randomTFBool) {
                            ...
                            g.setColor(Color.white);
                            g.setFont(new Font("Tahoma", Font.PLAIN, 17));
                            g.drawString("Please Enter An Integer:", 983, 837);
                        }
                    }
                }
                //display random cells
                for(int i = 0;i < userInputRandomTFInt;i++){
                    int xAxis = rand.nextInt(64);
                    int yAxis = rand.nextInt(36);
                    if(aliveInt[xAxis][yAxis] == 1){
                        i -= 1;
                    }else{
                        aliveInt[xAxis][yAxis] = 1;
                    }
                }
            }
        }
    ...
    //check the number of alive cells nearby method
    ...
    //setup mouse listener(moved/clicked) that gets user input
    ...
    public class Click implements MouseListener{
        ...
        public void mouseClicked(MouseEvent e) {
            ...
            //oscillators type button
            if(mouseX > 237 && mouseX < 437 && mouseY > 840 && mouseY < 900){
                oscillators = true;
            }
            //spaceship type button
            if(mouseX > 457 && mouseX < 657 && mouseY > 840 && mouseY < 900){
                spaceships = true;
            }
            //Gosper's glider gun button
            if(mouseX > 677 && mouseX < 967 && mouseY > 840 && mouseY < 900){
                gun = true;
            }
            //random button
            ...
            if(randomTFBool) {
                //run random button
                if (mouseX > 1137 && mouseX < 1187 && mouseY > 880 && mouseY < 900) {
                    userInputRandomTF = randomCellTF.getText();
                    randomRun = true;
                }
            }
    ...
    //check if mouse position is in cells
    public int inBoxX(){
        for(int i = 0;i < 64;i++){
            for(int j = 0;j < 36;j++){
                if(mouseX >= spacing+i*20+7 && mouseX < spacing+i*20+20+7-2*spacing && mouseY >= spacing+j*20+80+29 && mouseY < spacing+j*20+20+80+29-2*spacing){
